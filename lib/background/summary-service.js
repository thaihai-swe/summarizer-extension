(function () {
    ...
    const PDF_LONG_CONTENT_THRESHOLD = 60000;
    ...
    function shouldUseProgressiveSummarization(extracted) {
        ...
        if (sourceType === "pdf") {
            return sourceText.length > PDF_LONG_CONTENT_THRESHOLD;
        }
    ...
    function getSourceLabel(extracted) {
        ...
        if (extracted.sourceType === "pdf") {
            return extracted.isAcademic ? "academic paper" : "PDF document";
        }
    ...
    function getExtractionStatusLabel(tabUrl) {
        ...
        if (/arxiv\.org\/(abs|html|pdf)\/|pubmed|\.pdf($|\?)/i.test(url)) {
            return "Reading document & paper sections...";
        }
    ...
    async function generateSummaryText(extracted, settings, providerId, tabId) {
        ...
        if (shouldUseProgressiveSummarization(extracted)) {
            return summarizeWithBoundedChunking(extracted, settings, providerId, {
                tabId,
                maxRequests: extracted.sourceType === "course" ? COURSE_MAX_REQUESTS : extracted.sourceType === "pdf" ? PDF_MAX_REQUESTS : WEBPAGE_MAX_REQUESTS
            });
        }
    ...
    async function answerFollowUp(tabId, question, options) {
        ...
        console.log(`[SummaryService] answerFollowUp for tab ${tabId} with mode: ${grounding}`);
        const conversationHistory = await SummarizerStorage.getConversationForTab(tabId);
        const prompt = grounding === "open"
            ? SummarizerPrompts.buildOpenFollowUpPrompt(question, settings, conversationHistory)
