(function () {
    const CONTENT_SHELL_FILES = [
        "content-scripts/content.js"
    ];
    const EXTRACTOR_SCRIPT_FILES = [
        "extractors.js"
    ];
    ...
    async function sendExtractionMessage(tabId, messageType) {
        async function requestFromContent() {
            const response = await sendTabMessage(tabId, { type: messageType });
            if (response && response.code === "EXTRACTORS_NOT_READY") {
                await injectScripts(tabId, EXTRACTOR_SCRIPT_FILES);
                return sendTabMessage(tabId, { type: messageType });
            }
            return response;
        }

        try {
            const response = await requestFromContent();
            ...
            try {
                await injectScripts(tabId, CONTENT_SHELL_FILES.concat(EXTRACTOR_SCRIPT_FILES));
    ...
    async function openSidePanel() {
        try {
            ...
            throw new Error(error && error.message ? error.message : "The browser could not open the sidebar. Try the extension toolbar button.");
        }
    }
