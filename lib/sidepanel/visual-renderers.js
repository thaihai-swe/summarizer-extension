(function () {
    ...
    function renderConceptTree(text, host) {
        ...
        nodes.forEach((node) => {
            ...
            tier.className = "concept-tier";
            tier.textContent = node.tier;
            const term = document.createElement("h4");
            term.className = "concept-term";
            term.textContent = node.term;
            card.append(tier, term);
            if (node.definition) {
                const definition = document.createElement("p");
                definition.className = "concept-def";
                definition.textContent = node.definition.slice(0, 220);
    ...
    function renderTimelineRail(text, host) {
        ...
        events.forEach((event) => {
            ...
            if (event.detail) {
                ...
                detail.textContent = event.detail.slice(0, 280);
