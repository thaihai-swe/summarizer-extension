(function () {
    ...
    const RETIRED_KEYS = new Set(["density"]);
    ...
    const SECTIONS = {
        summary: {
            ...
            summaryTone: { type: "select", default: "Simple", valid: new Set(["Simple", "Concise", "Friendly", "Expert", "Professional", "Academic", "Critical", "Witty", "Roast"]) }
        },
        wordTarget: {
            customFormulaEnabled: { type: "boolean", default: false },
            sizeMultiplierBrief: { type: "number", default: 0.05, normalize: (v) => Math.max(0.01, Math.min(1.0, Number(v) || 0.05)) },
            sizeMultiplierMedium: { type: "number", default: 0.08, normalize: (v) => Math.max(0.01, Math.min(1.0, Number(v) || 0.08)) },
            sizeMultiplierDeep: { type: "number", default: 0.12, normalize: (v) => Math.max(0.01, Math.min(1.0, Number(v) || 0.12)) },
            lengthMultiplierShort: { type: "number", default: 0.7, normalize: (v) => Math.max(0.1, Math.min(5.0, Number(v) || 0.7)) },
            lengthMultiplierMedium: { type: "number", default: 1.0, normalize: (v) => Math.max(0.1, Math.min(5.0, Number(v) || 1.0)) },
            lengthMultiplierLong: { type: "number", default: 1.35, normalize: (v) => Math.max(0.1, Math.min(5.0, Number(v) || 1.35)) },
            charsPerWord: { type: "number", default: 5, normalize: (v) => Math.max(1, Math.min(20, Number(v) || 5)) },
            minTargetWords: { type: "number", default: 60, normalize: (v) => Math.max(10, Math.min(5000, Math.round(Number(v) || 60))) },
            maxTargetWords: { type: "number", default: 1500, normalize: (v) => Math.max(50, Math.min(20000, Math.round(Number(v) || 1500))) },
            customTargetTemplate: { type: "text", default: "", normalize: (v) => String(v || "").slice(0, 1000) }
        },
        display: {
    ...
    function normalizeSettings(raw) {
        ...
        Object.keys(raw).forEach((key) => {
            if (!(key in out) && !RETIRED_KEYS.has(key)) out[key] = raw[key];
        });
        return out;
    }
