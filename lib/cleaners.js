(function () {
    ...
    function resolveSection(heading) {
        ...
        if (/^(?:(?:suggested|example|sample|generated)\s+)?follow[- ]?up questions?$/.test(compact)
            || /^(?:suggested|example|sample|generated) questions?$/.test(compact)) {
    ...
    function parseStructuredSummary(rawText) {
        ...
        if (!followUpQuestions.length) {
            const questionsPattern = text.match(
                /(?:(?:suggested|example|sample|generated)\s+)?follow[- ]?up questions?\s*:?\s*\n([\s\S]*?)(?=\n\s*(?:#{1,6}\s+|\*\*[^*]+\*\*\s*:?\s*$|$))/i
            ) || text.match(
                /(?:suggested|example|sample|generated) questions?\s*:?\s*\n([\s\S]*?)(?=\n\s*(?:#{1,6}\s+|\*\*[^*]+\*\*\s*:?\s*$|$))/i
            );
            if (questionsPattern) {
