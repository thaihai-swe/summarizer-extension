(function () {
    ...
    function assessCoverageSignals(parsed, sourceText) {
        const output = parsed ? [
            ...
            Array.isArray(parsed.sections) ? parsed.sections.map((section) => section && section.content) : []
        ].flat().join("\n") : "";
    ...
    function buildQualityContract(context) {
        ...
        const isDeep = !isBrief && (size === "Deep" || length === "Long" || deepModes.has(mode));
        ...
        if (mode === "concepts") {
            return {
                ...
                isDeep,
                isBrief,
                required: ["conceptMap", "coreDefinitions", "prerequisitesMisconceptions"],
                recommended: ["practicalSteps", "pitfallsWarnings", "resourcesTools"]
            };
        }
        ...
        if (includesDeepSections || mode === "study") {
            recommended.push("reviewKit");
        }
        if (!isBrief && (sourceType === "course" || mode === "study" || mode === "analyze")) {
            recommended.push("practicalSteps");
        }

        if (!isBrief && mode === "study") {
            recommended.push("conceptMapAndPrerequisites");
            recommended.push("reviewKit");
        }
    ...
    function evaluateSummary(parsed, context) {
        ...
        }, 0) + (parsed && Array.isArray(parsed.sections)
            ? parsed.sections
                .filter((section) => section && !section.canonicalKey)
                .reduce((total, section) => total + contentLength(section.content), 0)
            : 0);
