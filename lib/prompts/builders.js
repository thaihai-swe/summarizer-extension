(function () {
    ...
    function withActivePreset(settings) {
        const source = settings || {};
        const mode = String(source.promptMode || "summarize").toLowerCase();
        const presets = Array.isArray(source.customPromptPresets) ? source.customPromptPresets : [];
        const preset = mode.startsWith("preset-") ? presets.find((item) => item.id === mode) : null;
        if (!preset) return Object.assign({}, source);
        return Object.assign({}, source, {
            customSystemInstructions: preset.systemPrompt || "",
            customPromptInstructions: preset.userPrompt || ""
        });
    }
    ...
    function buildSummaryPrompt(context, settings) {
        ...
        const presetSettings = withActivePreset(settings);
        const mode = String(presetSettings.promptMode || "summarize").toLowerCase();
        const deepModes = new Set(["analyze", "explain", "study", "debate", "concepts"]);
        const patchedSettings = Object.assign({}, presetSettings);
    ...
    function buildChunkSummaryPrompt(context, chunk, index, total, settings) {
        const patchedSettings = withActivePreset(settings);
        const mode = String(patchedSettings.promptMode || "summarize").toLowerCase();
        if (mode === "concepts") {
            return SummarizerCoursePromptTemplate.buildConceptsChunkPrompt(context, chunk, index, total, patchedSettings);
        }
        if (context.sourceType === "youtube") {
            return SummarizerYoutubePromptTemplate.buildYoutubeChunkPrompt(context, chunk, index, total, patchedSettings);
        }
        if (context.sourceType === "course") {
            return SummarizerCoursePromptTemplate.buildCourseChunkPrompt(context, chunk, index, total, patchedSettings);
        }
        if (context.sourceType === "pdf") {
            return SummarizerPdfPromptTemplate.buildPdfChunkPrompt(context, chunk, index, total, patchedSettings);
        }
        return SummarizerWebpagePromptTemplate.buildWebpageChunkPrompt(context, chunk, index, total, patchedSettings);
    }
    ...
    function buildSynthesisPrompt(context, chunkSummaries, settings) {
        ...
        const mode = String(patchedSettings.promptMode || "summarize").toLowerCase();
        if (mode === "concepts") {
            return SummarizerCoursePromptTemplate.buildConceptsSynthesisPrompt(context, chunkSummaries, patchedSettings);
        }
        if (context.sourceType === "youtube") {
            return SummarizerYoutubePromptTemplate.buildYoutubeSynthesisPrompt(context, chunkSummaries, patchedSettings);
        }
        if (context.sourceType === "course") {
            return SummarizerCoursePromptTemplate.buildCourseSynthesisPrompt(context, chunkSummaries, patchedSettings);
        }
        if (context.sourceType === "pdf") {
            return SummarizerPdfPromptTemplate.buildPdfSynthesisPrompt(context, chunkSummaries, patchedSettings);
        }
        return SummarizerWebpagePromptTemplate.buildWebpageSynthesisPrompt(context, chunkSummaries, patchedSettings);
    }
    ...
    function buildDeepDivePrompt(context, question, settings) {
        ...
        return common.buildPromptEnvelope({
            ...
            detailsSection: [
                ...
                `Connections, Causes & Tradeoffs:\n${common.cleanPromptValue(context.argumentAndInsight) || "None."}`,
    ...
    function buildOpenFollowUpPrompt(question, settings, conversationHistory) {
        const common = SummarizerPromptCommon;
        const conversation = Array.isArray(conversationHistory)
            ? conversationHistory.slice(-6)
            : [];

        return common.buildPromptEnvelope({
            role: "You are an expert AI assistant answering a general-knowledge follow-up question. You are not restricted to the summarized webpage or source material.",
            sourceType: "general",
            settings,
            customInstructions: settings.customPromptInstructions || "",
            customSystemInstructions: settings.customSystemInstructions || "",
            sourceContext: [
                `Current mode: ${common.getPromptMode(settings)}.`,
                "This follow-up is in General mode.",
                "Answer using your broad knowledge. Do not claim or imply that the answer comes from the current webpage, video, course, or document.",
                "If recent conversation includes Source-grounded turns, you may mention them only when they directly help. They are not evidence for this answer."
            ],
            task: [
                "Answer the user's question directly and clearly.",
                "Use general knowledge. Do not invent citations from the summarized source.",
                "Lead with the direct answer, then add concise supporting explanation.",
                "If you are uncertain, say so instead of fabricating specifics."
            ],
            sectionPlan: common.buildInternalSectionPlan("openFollowUp"),
            detailsSection: [
                conversation.length
                    ? `Recent Follow-up History:\n${common.renderConversationHistory(conversation, 6)}`
                    : "Recent Follow-up History:\nNone.",
