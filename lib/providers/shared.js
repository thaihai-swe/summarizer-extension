(function () {
    function getTimeoutMs(summaryLength, summarySize) {
        // Keep one generous ceiling for generation and stream-body reads.
        // Providers may still fail sooner on their own transport limits.
        return 300000;
    }
    ...
    async function readStream(response, onChunk, extractChunkText, signal, requestContext) {
        try {
            ...
            while (true) {
                ...
                while ((newlineIndex = buffer.indexOf("\n")) !== -1) {
                    ...
                    if (chunkText) {
                        fullText += chunkText;
                        if (requestContext) requestContext.streamedOutput = true;
            ...
            if (signal && signal.aborted) {
                const timeoutError = new Error("Request timed out while reading the provider response.");
